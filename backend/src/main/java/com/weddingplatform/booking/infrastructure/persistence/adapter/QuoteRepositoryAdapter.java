...
public class QuoteRepositoryAdapter implements QuoteRepository {
    ...
    @Override public Quote save(Quote q) {
        ...
        e.setUuid(q.uuid()); e.setWeddingId(q.weddingId()); e.setVendorServiceId(q.vendorServiceId());
        e.setRequestedById(q.requestedById()); e.setStatus(q.status()); e.setEventDate(q.eventDate());
        e.setGuestCount(q.guestCount()); e.setCustomRequirements(q.customRequirements());
        e.setSubtotal(q.subtotal()); e.setDiscountAmount(q.discountAmount());
        e.setTaxAmount(q.taxAmount()); e.setTotalAmount(q.totalAmount());
        e.setCurrencyCode(q.currencyCode()); e.setValidUntil(q.validUntil()); e.setNotes(q.notes());
    ...
    private Quote toDomain(QuoteEntity e) {
        return new Quote(e.getId(), e.getUuid(), e.getWeddingId(), e.getVendorServiceId(), e.getRequestedById(),
            e.getStatus(), e.getEventDate(), e.getGuestCount(), e.getCustomRequirements(),
            e.getSubtotal(), e.getDiscountAmount(), e.getTaxAmount(), e.getTotalAmount(),
            e.getCurrencyCode(), e.getValidUntil(), e.getNotes(), e.getCreatedAt());
    }
}
